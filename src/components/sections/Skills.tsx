...
import { Code2, Database, GitBranch, Box, Globe, Layers, Server } from "lucide-react";

const toolIcons: Record<string, React.ReactNode> = {
  PostgreSQL: <Database size={16} className="text-primary" />,
  "SQL Server": <Database size={16} className="text-primary" />,
  "Git & GitHub": <GitBranch size={16} className="text-primary" />,
  Docker: <Box size={16} className="text-primary" />,
  "APIs REST": <Globe size={16} className="text-primary" />,
};
...
export default function Skills() {
  ...
  const java = skills.primary[0];
  const spring = skills.primary[1];
  const git = skills.tools.find((s) => s.name === "Git & GitHub")!;
  const docker = skills.tools.find((s) => s.name === "Docker")!;
  const dbTools = skills.tools.filter((s) =>
    ["PostgreSQL", "SQL Server", "APIs REST"].includes(s.name)
  );

  const subtitle =
    locale === "pt"
      ? "Stack atual e trajetória técnica — Java & Spring Boot como foco principal."
      : "Current stack and technical background — Java & Spring Boot as the main focus.";
  const legacyLabel = locale === "pt" ? "Stack Anterior" : "Previous Stack";
  const legacyNote =
    locale === "pt"
      ? "Base técnica sólida, foco atual em Java"
      : "Solid technical foundation, current focus on Java";
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <SectionWrapper delay={0.08}>
          <div className="space-y-3">
            {/* Row 1: Java (large) + Spring Boot / Git / Docker */}
            <div className="grid grid-cols-1 md:grid-cols-[5fr_7fr] gap-3">
              {/* Java — large featured card */}
              <div className="group rounded-2xl bg-primary/[0.06] border border-primary/[0.15] p-6 sm:p-7 transition-all duration-300 hover:border-primary/[0.28] hover:bg-primary/[0.09] hover:shadow-[0_0_40px_rgba(138,123,255,0.08)] flex flex-col justify-between min-h-[200px]">
                <div>
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-xl font-bold tracking-tight">{java.name}</h3>
                  </div>
                  <p className="text-sm text-muted leading-relaxed">
                    {t(java.description, locale)}
                  </p>
                </div>
              </div>

              {/* Spring Boot + Git/Docker — nested */}
              <div className="grid gap-3" style={{ gridTemplateRows: "auto auto" }}>
                ...
                <div className="group rounded-2xl bg-primary/[0.05] border border-primary/[0.12] p-5 sm:p-6 transition-all duration-300 hover:border-primary/[0.24] hover:bg-primary/[0.08]">
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <h3 className="text-base font-bold tracking-tight">{spring.name}</h3>
                  </div>
                  <p className="text-[13px] text-muted leading-relaxed">
                    {t(spring.description, locale)}
                  </p>
                </div>

                {/* Git & Docker side by side */}
                <div className="grid grid-cols-2 gap-3">
                  <div className="group rounded-2xl bg-white/[0.015] border border-white/[0.05] p-4 transition-all duration-300 hover:border-primary/[0.18] hover:bg-white/[0.03]">
                    <div className="flex items-center gap-2.5 mb-2">
                      ...
                      <h3 className="text-sm font-semibold truncate">{git.name}</h3>
                    </div>
                  ...
                  <div className="group rounded-2xl bg-white/[0.015] border border-white/[0.05] p-4 transition-all duration-300 hover:border-primary/[0.18] hover:bg-white/[0.03]">
                    <div className="flex items-center gap-2.5 mb-2">
                      ...
                      <h3 className="text-sm font-semibold">{docker.name}</h3>
                    </div>
                    <span className="text-[10px] font-medium text-primary/60">
                      {t(docker.label, locale)}
                    </span>
                  </div>
                </div>
              </div>
            ...
            {/* Row 2: PostgreSQL, SQL Server, APIs REST */}
              ...
              {dbTools.map((tool) => (
                ...
                >
                  <div className="w-9 h-9 rounded-lg bg-primary/[0.08] border border-primary/[0.1] flex items-center justify-center flex-shrink-0">
                    {toolIcons[tool.name] ?? <Server size={16} className="text-primary" />}
                  </div>
